...
selected2 = st.selectbox('Choose what type of expert you want to work on the Solution doc:', ('None','Lead Solution Architect', 'Chief Technology Officer'), index=st.session_state['selection'])

if(st.session_state['sow_result']):

		if selected2 != 'None':
			create_solutiondoc = agents.create_solutiondoc(selected2)
			...
			taskx1 = tasks.task6(create_solutiondoc,st.session_state['sow_result'])
			...
			with st.expander("Processing!"):
			    sys.stdout = StreamToExpander2(st)
			    with st.spinner("Generating Results"):
			        crew_result2 = crew2.kickoff()
			...
			st.header("Results:")
			st.markdown(crew_result2)
			st.session_state['solution_doc'] = crew_result2
